var Mole = (function(){
    function Mole(normalState,hitState,downY,hitCallBackHd,scoreImg){
        ...
        //监听点击事件触发发送http请求
        this.normalState.on(Laya.Event.CLICK,this,this.testHttp);
    }
    var _proto = Mole.prototype;

    //测试http
    _proto.testHttp=function(){
        var xhr = new Laya.HttpRequest();
        var requestAddress="http://p12.local.com/wsBigMemberInterface/pages/saveOTORelation.jsx?name=oooo8889ooo&mid=vvvvvvvvvv&branchcode=kkkkkkkkkkk";
        //xhr.http.timeout = 100;//设置超时时间；
        ...
        //测试get方法
        xhr.send(requestAddress,"","get","json");
